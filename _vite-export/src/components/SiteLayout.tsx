          ...
          <nav className="hidden lg:flex items-center gap-8">
            {NAV.map(n => (
              <NavLink key={n.to} to={n.to} end={n.end}
                className={({ isActive }) =>
                  `text-xs tracking-[0.2em] uppercase transition-colors relative group ${isActive ? "text-gold" : "text-cream/80 hover:text-gold"}`
                }
              >
                {n.label}
          ...
          <div className="lg:hidden border-t border-gold/20 bg-ink/95">
            <nav className="flex flex-col p-6 gap-5">
              {NAV.map(n => (
                <NavLink key={n.to} to={n.to} end={n.end}
                  className={({ isActive }) =>
                    `text-sm tracking-[0.2em] uppercase ${isActive ? "text-gold" : "text-cream/80 hover:text-gold"}`
                  }
                >
                  {n.label}
                </NavLink>
